# Feature Specification: Design System (Blueprint Lab v5)
...
**Independent Test**: Verify that `custom-v5.css` is loaded on every page and that CSS custom properties for all design tokens are defined in `:root`.

**Acceptance Scenarios**:

1. **Given** a visitor loads any page, **When** the page renders, **Then** the custom design system (`custom-v5.css`) is applied on top of the tabi base theme.
...
- What happens if `custom-v5.css` fails to load (CDN issue)? The tabi base theme should still render a usable (if unstyled) site.
...
- **FR-005**: The design system MUST be contained in a single CSS file (`static/custom-v5.css`) loaded via `config.toml [extra] stylesheets`.
...
- **Component Styles** (selectors in `custom-v5.css`): Cards, tags, admonitions, hero, code blocks, buttons, navigation.
- **Theme Overrides** (`[data-theme="light"]`): Light mode token replacements.
- **Config Integration** (`config.toml`): `stylesheets = ["custom-v5.css"]`, `skin = "monochrome"`.

## Success Criteria

### Measurable Outcomes

- **SC-001**: `custom-v5.css` is present in the build output and loaded on every page.
